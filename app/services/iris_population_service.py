...
class IrisPopulationService:
    """Service population IRIS — supporte plusieurs millésimes."""

    # ── Millésimes disponibles (mis en cache 24h) ──────────────────────────────
    @lru_cache(maxsize=1)
    def get_available_years(self) -> list[int]:
        """Retourne la liste des millésimes disponibles en base, triés."""
        db = SessionLocal()
        try:
            rows = db.execute(
    ...
    def _resolve_year(self, year: Optional[int]) -> Optional[int]:
        """Retourne l'année demandée ou le dernier millésime disponible."""
        if year is not None:
            return year
        return self.get_latest_year()
    ...
    def get_by_iris(self, iris_code: str, year: Optional[int] = None) -> Optional[dict]:
        ...
        try:
            sql = text("""
                SELECT iris_code, com_code, year,
                       pop, pop_0_2, pop_3_5, pop_6_10, pop_11_17,
                       pop_18_24, pop_25_39, pop_40_54, pop_55_64,
                       pop_65_79, pop_80_plus, pop_foreign, pop_immigrant,
                       pop_women, pop_men
                FROM iris_population
                WHERE iris_code = :iris_code AND year = :year
                LIMIT 1
            """)
        ...
        try:
            sql = text("""
                SELECT iris_code, com_code, year,
                       ...
                       pop_women, pop_men
                FROM iris_population
                WHERE com_code = :com_code AND year = :year
                ORDER BY iris_code
            """)
    ...
    def get_by_epci(self, epci_code: str, year: Optional[int] = None) -> dict:
        ...
        try:
            sql = text("""
                SELECT ip.iris_code, ip.com_code, ip.year,
    ...
    def get_by_department(self, dep_code: str, year: Optional[int] = None) -> dict:
        ...
        try:
            sql = text("""
                ...
                FROM iris_population ip
                JOIN geo_codes gc ON gc.codgeo = ip.com_code
                WHERE gc.dep = :dep_code AND ip.year = :year
                ORDER BY ip.com_code, ip.iris_code
            """)
    ...
    def get_by_region(self, reg_code: str, year: Optional[int] = None) -> dict:
        ...
        try:
            sql = text("""
                ...
                FROM iris_population ip
                JOIN geo_codes gc ON gc.codgeo = ip.com_code
                WHERE gc.reg = :reg_code AND ip.year = :year
                ORDER BY ip.com_code, ip.iris_code
            """)
